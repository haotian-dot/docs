...
触发话题是指该规则的触发条件所需要的数据来源，即规则的触发条件会在这些话题的数据中进行判断。

添加触发话题的基础编辑操作如下：

- （前提）在组织设备配置中已经配置了组织设备的话题总话题列表。配置方式参见[设备配置](../3-device/4-device-collector.md#话题设置)
- 选择规则的触发话题（下图例：选择了`/velocity`话题）
- 点击右侧【+】按钮增加一个话题
- 选择新增的话题（下图例：选择了`/temp`话题）

![pro-rule-topics](../img/pro-rule-topics.png)
